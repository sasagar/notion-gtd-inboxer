const WarningConponent = (props) => {
    ...
    if (result == 200) {
        return (
            <div className="bg-green-600 border-green-700 text-green-100 font-bold w-80 p-2 rounded-sm">
                Inboxに追加しました。
            </div>
        )
    } else if (result == 4000) {
        ...
        return (
            <div className="bg-amber-600 border-amber-700 text-amber-100 font-bold w-80 p-2 rounded-sm">
                Notionとの接続で問題が起きました。
            </div>
        )
    } else if (result == 429) {
